...
void RestAPISystem::setup(RestAPIEndpoints &endpoints)
{
    ...
    // WiFi Portal endpoints
    endpoints.addEndpoint("wifiscan", RestAPIEndpointDef::ENDPOINT_CALLBACK, RestAPIEndpointDef::ENDPOINT_GET, 
                std::bind(&RestAPISystem::apiWifiScan, this, std::placeholders::_1, std::placeholders::_2), 
                "Scan for WiFi networks", "application/json");
    endpoints.addEndpoint("wifiportal", RestAPIEndpointDef::ENDPOINT_CALLBACK, RestAPIEndpointDef::ENDPOINT_GET, 
                std::bind(&RestAPISystem::apiWifiPortal, this, std::placeholders::_1, std::placeholders::_2), 
                "WiFi portal page", "text/html");
    }
...
// WiFi Portal methods
void RestAPISystem::apiWifiScan(String &reqStr, String &respStr)
{
    // Check if this is a request to start a new scan (query parameter ?start=1)
    bool startNewScan = reqStr.indexOf("start=1") >= 0;
    
    // Check if a scan is already in progress
    int networkCount = WiFi.scanComplete();
    
    if (networkCount == WIFI_SCAN_RUNNING) {
        respStr = "{\"scanning\":true,\"networks\":[]}";
        Log.trace("%sWiFi scan in progress...\n", MODULE_PREFIX);
        return;
    }
    
    // If requesting a new scan or no previous results, start a new scan
    if (startNewScan || networkCount == WIFI_SCAN_FAILED) {
        WiFi.scanDelete();
        WiFi.scanNetworks(true); // Start async scan
        respStr = "{\"scanning\":true,\"networks\":[]}";
        Log.notice("%sStarting WiFi scan...\n", MODULE_PREFIX);
        return;
    }
    
    // If networkCount is negative (but not WIFI_SCAN_RUNNING), it's an error
    if (networkCount < 0) {
        respStr = "{\"error\":true,\"networks\":[]}";
        Log.warning("%sWiFi scan failed with code %d\n", MODULE_PREFIX, networkCount);
        return;
    }
    // Build response with scan results (networkCount >= 0 means scan completed successfully)
    respStr = "{\"networks\":[";
    for (int i = 0; i < networkCount; i++) {
        if (i > 0) respStr += ",";
        respStr += "{";
        respStr += "\"ssid\":\"" + WiFi.SSID(i) + "\",";
        respStr += "\"rssi\":" + String(WiFi.RSSI(i)) + ",";
        respStr += "\"secure\":" + String(WiFi.encryptionType(i) != WIFI_AUTH_OPEN ? "true" : "false");
        respStr += "}";
    }
    respStr += "]}";
    
    Log.notice("%sWiFi scan found %d networks\n", MODULE_PREFIX, networkCount);
}

void RestAPISystem::apiWifiPortal(String &reqStr, String &respStr)
{
    // Return the WiFi portal HTML page
    respStr = _wifiManager.getPortalHTML();
}

void RestAPISystem::apiRootPage(String &reqStr, String &respStr)
{
    // Return portal page in portal mode, otherwise redirect to index.html
    if (_wifiManager.isPortalMode()) {
        respStr = _wifiManager.getPortalHTML();
    } else {
        respStr = "<!DOCTYPE html><html><head><meta http-equiv=\"refresh\" content=\"0;url=/index.html\"></head></html>";
    }
}

...
void FileManager::setup(ConfigBase& config, const char* pConfigPath)
{
    ...
    if (_enableSD)
    {
        ...
        else
        {
            ...
            esp_err_t retInit = spi_bus_initialize(SPI2_HOST, &bus_cfg, SDSPI_DEFAULT_DMA);
            if (retInit != ESP_OK){
                Log.warning("%sFailed to init SPI bus",MODULE_PREFIX);
                return;
            }
            sdspi_device_config_t slot_config = SDSPI_DEVICE_CONFIG_DEFAULT();
            slot_config.gpio_cs = static_cast<gpio_num_t>(sdCSPin);
            slot_config.host_id = SPI2_HOST;
            esp_err_t ret = esp_vfs_fat_sdspi_mount("/sd", &host, &slot_config, &mount_config, &pCard);
            if (ret != ESP_OK){
                if (ret == ESP_FAIL)
                {
                    Log.warning("%sfailed to mount SDcard\n",MODULE_PREFIX);
                } else {
                    Log.warning("%sfailed to init SDcard (error %s)\n",MODULE_PREFIX,esp_err_to_name(ret));
                }
            } else {
                Log.info("%sSD details:\n", MODULE_PREFIX);
                // Default to SD
                _pSDCard = pCard;

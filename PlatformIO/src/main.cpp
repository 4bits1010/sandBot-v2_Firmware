...
void setup()
{
    ...
    // Try to load network config from SD card (after setup so _pConfigBase is available)
    bool sdConfigLoaded = wifiManager.loadNetworkConfigFromSD(fileManager);
    if (sdConfigLoaded)
    {
    ...
    // Add root endpoint for portal mode (will redirect to index.html in normal mode)
    restAPIEndpoints.addEndpoint("", RestAPIEndpointDef::ENDPOINT_CALLBACK, RestAPIEndpointDef::ENDPOINT_GET, 
                    std::bind(&RestAPISystem::apiRootPage, &restAPISystem, std::placeholders::_1, std::placeholders::_2), 
                    "Root page", "text/html");
...
void loop()
{
    ...
    if (wifiManager.isConnected() || wifiManager.isPortalMode())
    {
        ...
        // show HW LED for a power-on state, once WIFI.active blinkLED
